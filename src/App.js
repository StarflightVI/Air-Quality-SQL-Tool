const handleFileUpload = async (file) => {
  ...
  if (file.size > 1024 * 1024 * 1024) {
    const proceed = window.confirm(
      `This file is ${fileSizeGB} GB which is very large.\n\n` +
      `The app will load a sample of ~500,000 rows to prevent browser crashes.\n\n` +
      `Click OK to proceed with sample, or Cancel to choose a smaller file.`
    );
    if (!proceed) {
      return;
    }
  ...
  // For files > 1GB, only keep first 500k rows
  // For files 500MB-1GB, keep 100k rows
  // For smaller files, keep everything
  const maxRowsToKeep = file.size > 1024 * 1024 * 1024 ? 500000 : 
                       file.size > 500 * 1024 * 1024 ? 100000 : 
                       Infinity;
  ...
  try {
    Papa.parse(file, {
      ...
      chunk: (results, parser) => {
        chunkCount++;
        console.log(`Processing chunk ${chunkCount}, rows so far: ${allData.length}`);
        
        // Add rows until we reach the limit
        if (allData.length < maxRowsToKeep) {
          const rowsNeeded = maxRowsToKeep - allData.length;
          const rowsToAdd = results.data.slice(0, rowsNeeded);
          allData.push(...rowsToAdd);
        }
        
        // Stop reading once we have enough data
        if (allData.length >= maxRowsToKeep) {
          console.log(`Reached ${maxRowsToKeep} rows limit, stopping parse`);
          parser.abort();
        }
      },
      complete: (results) => {
        ...
        if (allData.length === 0) {
          setError('No data found in CSV file. The file may be empty or corrupted.');
          setLoading(false);
          return;
        }

        const cleanHeaders = results.meta.fields.map(h => h ? h.trim() : '');
        setHeaders(cleanHeaders);
        setCsvData(allData);
        setQueryResult(null);
        setLoading(false);
        
        let message = `Successfully loaded ${allData.length.toLocaleString()} rows with ${cleanHeaders.length} columns`;
        if (allData.length === maxRowsToKeep) {
          message += `\n\nNote: Loaded first ${maxRowsToKeep.toLocaleString()} rows from this large file for performance.`;
        }
        
        console.log(message);
        
        // Show success message
        if (allData.length === maxRowsToKeep && file.size > 500 * 1024 * 1024) {
          setTimeout(() => {
            alert(`✓ File loaded successfully!\n\nLoimport React, { useState, useRef } from 'react';
...
const handleFileUpload = async (file) => {
  ...
  // For very large files, limit the data we keep in memory
  const maxRowsToKeep = file.size > 500 * 1024 * 1024 ? 100000 : Infinity;
  let rowsProcessed = 0;
  const allData = [];
  ...
  try {
    Papa.parse(file, {
      ...
      chunk: (results, parser) => {
        ...
        if (allData.length >= maxRowsToKeep) {
          parser.abort(); // Stop reading if we have enough data
          console.log(`Loaded ${maxRowsToKeep} rows (stopped early for memory)`);
        }
      },
      complete: (results) => {
        console.log('Parse complete:', rowsProcessed, 'rows processed');
        
        const dataToUse = allData.length > 0 ? allData : results.data;
        ...
        const cleanHeaders = results.meta.fields.map(h => h ? h.trim() : '');
        setHeaders(cleanHeaders);
        setCsvData(dataToUse);
        setQueryResult(null);
        setLoading(false);
        
        if (dataToUse.length === maxRowsToKeep) {
          alert(`Note: This file is very large. Loaded first ${maxRowsToKeep.toLocaleString()} rows for performance.`);
        }
        
        console.log(`Successfully loaded ${dataToUse.length} rows with ${cleanHeaders.length} columns`);
      },
      error: (error) => {
        console.error('Parse error:', error);
        
        // If we got some data before the error, use it
        if (allData.length > 0) {
          console.log('Error occurred but recovered with partial data:', allData.length, 'rows');
          setError(`Warning: File partially loaded (${allData.length} rows). Some data may be missing.`);
          setCsvData(allData);
          setLoading(false);
          return;
        }
        
        setError(`File parsing error: ${error.message || 'Unable to read file. The file may be too large, corrupted, or not a valid CSV.'}`);
        setLoading(false);
      }
    });
  } catch (err) {
